...
```{r}
figure3 <- "scanned Y values" |>
  read.csv(header = FALSE) |> 
  setNames("logCFU") |> 
  as_tibble() |> 
  mutate(group = rep(c("C", treatments), each = 4),
         cfu   = 10^logCFU) |> 
  select(-logCFU)
```
...
```{r}
x_lab <- c("C", treatments)
x_val <- seq_along(x_lab)
col_val <- rep(c(1, col_treatments), each = 4)

figure3b <- mutate(figure3, x = jitter(rep(x_val, each = 4)))

with(figure3b,
     plot(x, cfu, col = col_val, log = "y", ylim = c(1, 12000), xlab = NA, axes = FALSE,
          lwd = line_width, ylab = "number of CFU after 1 month of treatment"))
abline2(h = unlist(map(10^(0:3), ~ .x * 1:10)))

with(figure3b, points(x, cfu, col = col_val))
figure3b |> 
  filter(group != "C") |>
  with(points(x, cfu, pch = 19, col = adjustcolor(col_val[-(1:4)], .2)))

axis(1, x_val, x_lab); axis(2)
```
