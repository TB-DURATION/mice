...
c(est_RIF, est_RIFINH, est_RPTINH) |> 
  map(coef) |> 
  walk2(col_treatments[2:4], add_area)

legend2("topright", legend = treatments[-1], col = col_treatments[2:4], lwd = 2)
```
...
predicted_cfu <- function(t_max = 6 * 30, agg = 7, log = FALSE, cfu0 = 1e4, ...) {
  ...
  list(est_RIF, est_RIFINH, est_RPTINH) |> 
    map(coef) |> 
    walk2(col_treatments[2:4], add_pred, t_max, cfu0, aggregation = agg)
}
```

Let's try it:

```{r dependson = "predicted_cfu", fig.width = 1.7 * 5, fig.height = 3.1}
...
add_mod_pred(coef(est_RIF), col_treatments[2])
add_mod_pred(coef(est_RIFINH), col_treatments[3])
add_mod_pred(coef(est_RPTINH), col_treatments[4])
...
```{r fig.width = 1.7 * 5, fig.height = 3.1}
#| label: fig-effect-of-initial-cfu
#| fig-cap: "decay dynamics of CFU as a function of initial CFU value."

opar <- par(mfrow = c(1, 3), cex = 1, plt = c(.2, .95, .1983, .8947))
walk(10^(4:2), ~ predicted_cfu(xlab = "time (weeks)", log = TRUE, cfu0 = .x))
add_legend()
par(opar)
```

Another version of the `predicted_cfu2()` function:
...
Another way to look at the effect of initial CFU on the decay:

```{r dependson = "predicted_cfu2", fig.width = 1.7 * 5, fig.height = 3.1}
#| label: fig-effect-of-initial-cfu2
#| fig-cap: "another way to look at the decay dynamics of CFU as a function of initial
#| CFU value."

predicted_cfu2_tmp <- function(...) {
  predicted_cfu2(..., log = TRUE, xlab = "time (weeks)")
}

opar <- par(mfrow = c(1, 3), cex = 1, plt = c(.2, .95, .1983, .8947))

predicted_cfu2_tmp(est_drug = est_RIF,    col = col_treatments[2])
title2("RIF")
predicted_cfu2_tmp(est_drug = est_RIFINH, col = col_treatments[3])
title2("RIF+INH")
predicted_cfu2_tmp(est_drug = est_RPTINH, col = col_treatments[4])
title2("RPT+INH")
...
```{r}
add_speculation2 <- function(v_val = unlist(map(10^(0:3), ~ .x * 1:10)),
                             h_val = 10 * 0:20) {
  abline2(v = v_val); abline2(h = h_val)
  axis(1); axis(2)
  walk2(list(est_RIF, est_RIFINH, est_RPTINH), col_treatments[2:4], add_speculation1)
}
```
...
add_speculation2(v_val = seq(0, 50, 5))

plot_tmp(NA, xlim = c(1, 10), ylim = c(0, 50))
add_speculation2(h_val = seq(0, 50, 5))
