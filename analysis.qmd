...
The following function shows the model-predicted expected proportions of positive
individuals in the population:

```{r predicted_cfu, dependson = "add_pred"}
predicted_cfu <- function(t_max = 6 * 30, aggregation = 7, log = FALSE, cfu0 = 1e4, ...) {
  if (log) {
  ...
  add_pred(coef(est_RIF),    col_treatments[2], t_max, cfu0, aggregation = aggregation)
  add_pred(coef(est_RIFINH), col_treatments[3], t_max, cfu0, aggregation = aggregation)
  add_pred(coef(est_RPTINH), col_treatments[4], t_max, cfu0, aggregation = aggregation)
